...
import org.apache.spark.sql.types.{DataType, MyAstBuilder, StructType}
 ...
 * Concrete SQL parser for Catalyst-only SQL statements.
 */
class CatalystSqlParser(conf: SQLConf) extends MyAbstractSqlParser {
  val astBuilder = new MyAstBuilder(conf)
}

/** For test-only. */
object CatalystSqlParser extends MyAbstractSqlParser {
  val astBuilder = new MyAstBuilder(new SQLConf())
}
