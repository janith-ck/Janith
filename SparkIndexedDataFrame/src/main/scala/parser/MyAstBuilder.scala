...
import org.apache.spark.sql.catalyst.parser.{ParseException, SqlBaseBaseVisitor, SqlBaseParser}
...
class MyAstBuilder(conf: SQLConf) extends SqlBaseBaseVisitor[AnyRef] with Logging {
  ...
  override def visitTinyIntLiteral(ctx: TinyIntLiteralContext): Literal = {
    numericLiteral(ctx, 0, Byte.MaxValue, ByteType.simpleString)(_.toByte) //todo
  }
  ...
  override def visitSmallIntLiteral(ctx: SmallIntLiteralContext): Literal = {
    numericLiteral(ctx, 0, Short.MaxValue, ShortType.simpleString)(_.toShort) //todo
  }
